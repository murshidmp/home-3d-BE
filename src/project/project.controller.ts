...
export class ProjectController {
  ...
  @Get(':id/versions')
  @HttpCode(HttpStatus.OK)
  @ApiResponse({ status: 200, description: 'List of project versions.' })
  async getVersions(@Req() req: any, @Param('id') id: number) {
    const userId = req.user['sub'];
    const project = await this.projectService.getProjectById(userId, id);
    return ApiSuccessResponse.of(project.versions, 'Project versions fetched');
  }
